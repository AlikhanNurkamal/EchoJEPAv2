...
# -----------------------------
# Parallel worker (must be top-level for ProcessPoolExecutor pickling)
# -----------------------------
def _process_row(args: tuple):
    """
    Worker executed in a subprocess. Processes one DICOM, writes the result
    to a temp .npy file, and returns its path — avoiding large array IPC.

    Returns (dicom_uuid, tmp_path_or_None, err_tag_or_None, row_meta).

    "err_tag" is one of:
      - None            -> success, tmp_path is a valid file path
      - "not_a_dir"     -> disk_path does not exist
      - "empty"         -> processing returned no frames
      - "error:<msg>"   -> an exception was raised
    """
    (
        dicom_uuid, disk_path_str,
        src_fps, src_spacing,
        target_fps, target_spacing, target_size,
        study_uuid, view, manufacturer, n_frames,
    ) = args
    ...
    if not disk_path.is_dir():
        return dicom_uuid, None, "not_a_dir", row_meta
    ...
    try:
        ...
        if result is None or len(result) == 0:
            return dicom_uuid, None, "empty", row_meta

        # Write to a temp file instead of sending the large array over IPC pipe
        result_rgb = result[..., ::-1].copy()
        tmp = tempfile.NamedTemporaryFile(suffix=".npy", delete=False)
        np.save(tmp, result_rgb)
        tmp.close()
        return dicom_uuid, tmp.name, None, row_meta

    except Exception as exc:
        return dicom_uuid, None, f"error:{exc}", row_meta
...
def main():
    ...
    max_workers = args.max_workers or min(8, os.cpu_count() or 1)
    print(f"Using {max_workers} parallel workers.")

    # ---- Build task list (skip already-done here) ----
    # All metadata needed at write-time is embedded in the task tuple so we
    # never need to cache full DataFrame rows — saves several GB of RAM.
    tasks = []
    ...
    for row in df.itertuples(index=False):
        ...
        tasks.append((
            dicom_uuid,
            str(getattr(row, "disk_path", "")).strip(),
            src_fps, src_spacing,
            args.target_fps, args.target_spacing, args.target_size,
            str(getattr(row, "study_uuid", "")),
            str(getattr(row, "view", "")),
            str(getattr(row, "manufacturer", "")),
            int(getattr(row, "n_frames", 0)),
        ))
    print(f"  Total tasks to process: {len(tasks):,}")

    # Free the DataFrame — all needed data is now in tasks
    del df

    if not tasks:
        print("No tasks to process — all DICOMs already in progress file or none found.")
        writer.close()
        return

    # ---- Submit tasks in a bounded window; write results as they finish ----
    # Submitting all ~4M tasks at once would stall for minutes allocating Future
    # objects. Instead keep at most (max_workers * 4) in-flight at any time.
    print(f"\nProcessing DICOMs and writing shards (using {max_workers} workers)...")
    max_inflight = max_workers  # 1 task queued per worker; avoids disk contention from over-queuing
    task_iter = iter(tasks)
    in_flight: dict = {}  # future -> dicom_uuid

    # Use 'spawn' (not default 'fork') to avoid OpenCV/OpenMP thread-lock
    # deadlocks and to let each worker use full OpenCV internal threading.
    mp_context = mp.get_context("spawn")
    with ProcessPoolExecutor(max_workers=max_workers, mp_context=mp_context) as pool:
        def _submit_next():
            """Submit one more task from task_iter into the pool."""
            task = next(task_iter, None)
            if task is None:
                return
            fut = pool.submit(_process_row, task)
            in_flight[fut] = task[0]

        # Prime the pool with the initial window of tasks
        for _ in range(max_inflight):
            _submit_next()

        while in_flight:
            future = next(as_completed(in_flight))
            dicom_uuid = in_flight.pop(future)
            _submit_next()  # Refill: keep the pipeline full

            dicom_uuid_out, tmp_path, err_tag, row_meta = future.result()

            if err_tag == "not_a_dir":
                skipped_problem += 1
            ...
            if err_tag is not None:  # "error:<msg>"
                errors += 1
                append_progress(progress_path, dicom_uuid, "error")
                if errors <= 10:
                    print(f"  [error] {dicom_uuid}: {err_tag[6:]}")
                continue

            # Read frames from temp file written by worker, then clean up
            try:
                result_rgb = np.load(tmp_path)
            finally:
                Path(tmp_path).unlink(missing_ok=True)

            # Lookup text annotations for this DICOM and its study
            study_uuid = row_meta["study_uuid"]
            src_fps = row_meta["src_fps"]
            src_spacing = row_meta["src_spacing"]
