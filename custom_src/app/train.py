...
def main(args, resume_preempt=False):
    ...
    if use_wandb:
        wandb.init(
            project=cfgs_wandb.get("project", "echojepa-pretrain"),
    ...
    elif rank == 0 and not _WANDB_AVAILABLE:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if use_wandb:
                epoch_log = {
                    ...
                    "epoch/data_load_time_avg_ms": data_elapsed_time_meter.avg,
                    "epoch/gpu_mem_peak_mb": torch.cuda.max_memory_allocated() / 1024.0**2,
                    "epoch/number": epoch + 1,
                }
                for fpc_key, fpc_meter in mask_meters.items():
                    epoch_log[f"epoch/mask_fpc_{fpc_key}_fill_ratio"] = fpc_meter.avg
                wandb.log(epoch_log, step=(epoch + 1) * ipe)
